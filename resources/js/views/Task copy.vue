<template>
    ...
    <div class="sm:ml-64 mt-24 md:mt-[90px] px-4 md:px-10 h-fit w-full md:w-[84%] bg-custom-bg z-10 absolute">
        <!-- Breadcrumb Menu -->
        ...
        <div class="w-full h-fit mt-4 grid gap-2 md:gap-0 md:flow-root">
            <div class="w-fit h-fit grid float-left">
                ...
                <p class="w-fit text-[14px] md:text-[16px] text-custom-light-gray">Here you can see details regarding {{ carriers[errors[selected_error].carrier].name }}'s error on application {{ app_id }},</p>
            </div>
        ...
        <div class="w-full h-fit grid md:flex gap-2 md:gap-6 mt-6">
            ...
            <!-- Status -->
            <img :src="`/images/${errors[selected_error].status}.png`" alt="Error Status Bar" class="hidden md:block h-[47px] my-auto">
        </div>
        ...
        <div class="w-full h-fit grid grid-cols-4 gap-12 mt-8">
            ...
            <div class="grid col-span-3 gap-4 h-fit mb-8 p-6 bg-white border-[1px] border-custom-light rounded-[4px]">
                ...
                <!-- Error Test -->
                <div v-if="errors[selected_error].status == 'test'" class="w-fit h-fit flex gap-6">
